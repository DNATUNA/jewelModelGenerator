...
void GetFDoG(imatrix& image, ETF& e, double sigma, double sigma3, double tau, mymatrix * pEdgeField) 
{
	int	i, j;
	...
	//pEdgeField = new mymatrix(image_x, image_y);
	mymatrix dog(image_x, image_y);

	GetDirectionalDoG(image, e, dog, GAU1, GAU2, tau);
	GetFlowDoG(e, dog, (*pEdgeField), GAU3);

	for (i = 0; i < image_x; i++)
	{
		for (j = 0; j < image_y; j++)
		{
			image[i][j] = round((*pEdgeField)[i][j] * 255.);
		}
	}
}
...
mymatrix * GetFDoGRAW(imatrix& image, ETF& e, double sigma, double sigma3, double tau) 
{
	int image_x = image.getRow();
	int image_y = image.getCol();
	...
	mymatrix * tmp = new mymatrix(image_x, image_y);
	mymatrix dog(image_x, image_y);
